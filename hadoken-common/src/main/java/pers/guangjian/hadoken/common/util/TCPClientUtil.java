package pers.guangjian.hadoken.common.util;
...
/*        new Thread(() -> {
            try {
                for (int i = 0; i < 100; i++) {
                    ...
                    String s2 = modbusSendAndReceiveData("127.0.0.1", 6001, command);
                    if (!command.equalsIgnoreCase(s2)) {
                        System.out.println("thread2 - 心跳响应不正确 ========== 发送：" + command + "，接收：" + s2);
                    }
                    TimeUnit.SECONDS.sleep(3);
                }
            } catch (Exception e) {
