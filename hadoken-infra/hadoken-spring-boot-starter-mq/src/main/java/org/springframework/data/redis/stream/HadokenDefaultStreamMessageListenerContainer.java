...
/**
 * @Author: yanggj
 * @Description: 拓展 DefaultStreamMessageListenerContainer 实现，解决 Spring Data Redis + Redisson 结合使用时，Redisson 在 Stream 获得不到数据时，返回 null 而不是空 List，导致 NPE 异常。
...
public class HadokenDefaultStreamMessageListenerContainer<K, V extends Record<K, ?>> extends DefaultStreamMessageListenerContainer<K, V> {
    ...
    public static <K, V extends Record<K, ?>> StreamMessageListenerContainer<K, V> create(RedisConnectionFactory connectionFactory, StreamMessageListenerContainer.StreamMessageListenerContainerOptions<K, V> options) {
        ...
        return new HadokenDefaultStreamMessageListenerContainer<>(connectionFactory, options);
    }

    public HadokenDefaultStreamMessageListenerContainer(RedisConnectionFactory connectionFactory, StreamMessageListenerContainerOptions<K, V> containerOptions) {
